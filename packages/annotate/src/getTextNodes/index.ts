...
export class TextNodesFromDOM {
	...
	watch = (watchCB = console.info) => {
		// mutation listener
		const mutationFilter = (mutations: MutationRecord[], observer: MutationObserver) => {
			for (const mutation of mutations) {
				if (mutation.type === 'childList') {
					...
					if (addedNodes.length > 0) watchCB(addedNodes);
				}
			}
		};
		...
		document.addEventListener(
			...
			throttle(() => {
				...
				if (diff > 0.5) {
					this.endWatch(mutationObserver);
