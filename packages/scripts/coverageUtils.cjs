...
function convertJsonToMD(coverageJSON) {
    ...
    return coverageJSONList
       .map((kv, i) => {
        ...
        if (!i) { const overallPercentage = (lines.pct + statements.pct + functions.pct + branches.pct) / 4; const titleString = `## Overall Coverage of ${overallPercentage}%`; if (coverageJSONList.length === 1) {     return `${titleString}\n## No files defined for coverage were changed in this PR`; } return titleString;
        }
...
function removeKeysFromJson(coverageJSON, changedFiles) {
    return Object.entries(coverageJSON).reduce((acc, cur) => {
        ...
        const changedFileMatchesCleanedPathOrTotal = cur[0] === 'total' || changedFiles.some((filePath) => filePath.split('src/')[1] === cleanedPath);
        if (changedFileMatchesCleanedPathOrTotal) { return {...acc, [cur[0]]: cur[1] };
        }
        return acc;
    }, {});
}
